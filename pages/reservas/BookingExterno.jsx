...
export default function BookingExterno() {
  ...
  useEffect(() => {
    if (professionals.length === 1 && !selectedDay) {
      // Auto-seleccionar el único profesional
    }
  }, [professionals]);
  ...
  const nombreProfesional = professionals[0]?.name || scope;
  const especialidad      = professionals[0]?.specialty || "";
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc" }}>
      ...
      <div style={{
        ...
        <div>
          ...
          {especialidad && (
            <div style={{ fontSize: 13, color: "#64748b" }}>{especialidad}</div>
          )}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "20px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ fontSize: 14, color: "#64748b" }}>
            {selectedDay ? "Selecciona un horario disponible" : "Selecciona un día disponible"}
          </div>
          {selectedDay && (
            <button
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", color: "#94a3b8", padding: 40 }}>Sin agenda disponible.</div>
        ) : !selectedDay ? (
          <AgendaSummarySelector
            professionals={professionals}
            onSelectDay={setSelectedDay}
            preselectedId={professionals[0]?.id}
          />
        ) : (
          <>
            <AgendaDayController
