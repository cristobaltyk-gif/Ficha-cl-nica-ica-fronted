...
function ModalNueva({ onClose, onCreada, internalUser }) {
  ...
  const authHeaders = { "Content-Type": "application/json", "X-Internal-User": internalUser };
  ...
  if (linkPago) return (
    <div style={styles.overlay}>
      <div style={styles.modal}>
        ...
        <p style={{ color: "#475569", fontSize: 13, marginBottom: 16 }}>Link de pago para el centro:</p>
...
function ModalDescuento({ suscripcion, onClose, onGuardado, internalUser }) {
  ...
  return (
    <div style={styles.overlay}>
      <div style={styles.modal}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button style={{ ...styles.btnPrimary, flex: 1 }} onClick={handleGuardar} disabled={saving}>{saving ? "Guardando…" : "Guardar descuento"}</button>
          <button style={styles.btnSecondary} onClick={onClose}>Cancelar</button>
        </div>
      </div>
    </div>
...
export default function SuscripcionesAdmin() {
  ...
  async function handleCobrar(s) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/suscripciones/${s.centro_id}/cobrar`, {
        method: "POST", headers: { "X-Internal-User": internalUser }
      });
      const data = await res.json();
      if (data.link_pago) {
    ...
    <div style={{ padding: "16px 16px 40px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={k.label} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "12px 14px", textAlign: "center" }}>
      ...
      {loading ? (
        ...
        suscripciones.map(s => {
          const diasRestantes = s.fecha_vencimiento
            ...
            : null;
          return (
            <div key={s.centro_id} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#475569", marginBottom: 10, flexWrap: "wrap", gap: 4 }}>
                <span>💰 {fmt(s.precio_final)}/mes
                  {s.descuento_pct > 0 && <span style={{ color: "#16a34a", fontWeight: 600 }}> ({s.descuento_pct}% dto)</span>}
                </span>
                {diasRestantes !== null && (
              ...
              {s.descuento_hasta && (
                <div style={{ fontSize: 11, color: "#f97316", marginBottom: 8 }}>
                  ⏰ Descuento hasta {s.descuento_hasta} — {s.descuento_motivo}
                </div>
              )}
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button style={styles.btnSmall} onClick={() => handleCobrar(s)} disabled={cobrando === s.centro_id}>
                  {cobrando === s.centro_id ? "…" : "💳 Cobrar"}
                </button>
                <button style={styles.btnSmall} onClick={() => setModalDesc(s)}>🏷️ Descuento</button>
              </div>
            </div>
          );
        })
      ...
      {modalNueva && <ModalNueva onClose={() => setModalNueva(false)} onCreada={cargar} internalUser={internalUser} />}
      {modalDesc  && <ModalDescuento suscripcion={modalDesc} onClose={() => setModalDesc(null)} onGuardado={cargar} internalUser={internalUser} />}
    </div>
  );
}
...
const styles = {
  ...
  btnSmall: { background: "#f8fafc", color: "#0f172a", border: "1px solid #e2e8f0", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" },
