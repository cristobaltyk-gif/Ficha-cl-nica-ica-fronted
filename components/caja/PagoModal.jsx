...
const TIPOS_GRATUITOS = new Set(["control_gratuito"]);
...
export default function PagoModal({ open, slot, onClose, onSuccess, usuarioActual }) {
  ...
  useEffect(() => {
    if (!open || !slot) return;
    const prof = slot.professional || "";
    fetch(`${API_URL}/api/caja/config?professional=${prof}`)
      .then(r => r.ok ? r.json() : {})
  ...
  function handleTipo(val) {
    setTipo(val); setMetodo("efectivo"); setNumOp(""); setBanco(""); setError(null);
  }
  ...
  async function handlePagar() {
    ...
    if (!esGratuito && necesitaOp && !numOp.trim()) {
      setError("Ingresa el número de operación"); return;
    }
      ...
      <div style={s.modal}>
              ...
              {necesitaOp && (
                <div style={s.extraFields}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} type="text" placeholder="Ej: 123456789"
                      value={numOp} onChange={e => setNumOp(e.target.value)} maxLength={20}/>
                  </div>
                  <div style={s.fieldGroup}>
                    <label style={s.fieldLabel}>Banco origen</label>
                    <input style={s.input} type="text" placeholder="Ej: Banco Chile"
                      value={banco} onChange={e => setBanco(e.target.value)} maxLength={40}/>
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={s.footer}>
          ...
          <button style={s.btnPagar} onClick={handlePagar} disabled={loading}>
            {loading ? "Guardando…" : esControl ? "📧 Enviar confirmación" : esGratuito ? "✓ Confirmar" : `✓ Cobrar ${formatMonto(monto)}`}
          </button>
        </div>
