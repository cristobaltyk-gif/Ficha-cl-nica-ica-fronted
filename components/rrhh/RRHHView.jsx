  ...
  return (
    <div style={ms.backdrop}>
      <div style={ms.modal}>
        ...
        <div style={ms.body}>
          <Field label="Nombre *"><input style={ms.input} value={form.nombre} onChange={e => set("nombre", e.target.value)} /></Field>
          <Field label="RUT *"><input style={ms.input} value={form.rut} onChange={e => set("rut", e.target.value)} placeholder="12.345.678-9" /></Field>
          <Field label="Cargo">
          ...
          <Field label="Sueldo base / Honorarios *"><input style={ms.input} type="number" value={form.sueldo_base} onChange={e => set("sueldo_base", e.target.value)} placeholder="0" /></Field>
            ...
            <>
              ...
              {form.isapre && (
                <Field label="Monto Isapre ($)">
                  <input style={ms.input} type="number" value={form.monto_isapre} onChange={e => set("monto_isapre", e.target.value)} />
                </Field>
              )}
            </>
          )}
...
const ms = {
  ...
  modal:     { background: "#fff", borderRadius: 14, width: "100%", maxWidth: 400, fontFamily: "'DM Sans', system-ui", maxHeight: "90vh", overflowY: "auto" },
  header:    { background: "#0f172a", padding: "16px 20px", borderRadius: "14px 14px 0 0", position: "sticky", top: 0 },
  ...
  footer:    { display: "flex", gap: 10, padding: "14px 20px", borderTop: "1px solid #f1f5f9", position: "sticky", bottom: 0, background: "#fff" },
...
export default function RRHHView({
  ...
  function startEditTasas() {
    setTasasForm(JSON.parse(JSON.stringify(tasas)));
    setTasasEdit(true);
  }
          ...
          {trabajadores.map(t => (
            <div key={t.id} style={v.row}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={v.rowMeta}>{t.cargo} · {t.tipo_contrato.replace("_"," ")} · {fmt(t.sueldo_base)}</p>
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {!loading && liquidaciones && (
            <>
              ...
              {/* Tabla liquidaciones */}
              {liquidaciones.liquidaciones.map((liq, i) => (
      ...
      {tab === "tasas" && tasas && (
        <div style={v.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            {!tasasEdit
              ? <button style={v.btnSecondary} onClick={startEditTasas}>Editar</button>
          ...
          {Object.entries(tasas.afp || {}).map(([afp, tasa]) => (
            <div key={afp} style={v.row}>
              ...
              {tasasEdit
                ? <input style={{ ...ms.input, width: 80 }} type="number" step="0.0001"
          ...
          <div style={v.row}>
            ...
            {tasasEdit
              ? <input style={{ ...ms.input, width: 100 }} type="number"
                  value={tasasForm.utm}
